...
\pdfcomment[author=Charlles]{Aqui deveria ser b_{ij} no lugar de b?}
...
\hl{Gabriel: Este r é a distância entre duas partículas em coordenadas cartesianas, entretanto, na gênese do método se considera que o potencial no qual as partículas interagem é esfericamente simétrico, deve-se incluir também esta informação?
	
Charlles: É por isso que estranho a ausência de um termo $4\pi r^2$ nas integrais definidas abaixo. Eu esperaria uma integração feita no volume, o que em coordenadas esféricas dependeria de r, phi e theta. Porém, a consideração de interação esfericamente simétrica faria com que a integração em phi e theta pudesse ser feita analiticamente, resultando em $4\pi r^2$.

Charlles2: Já que você cita 'cartesian coordinates', o correto não seria center-to-center molecular distance vector? Isso significaria que a mudança para coordenadas esféricas e integração em phi e theta ainda não foi feita.}
...
\pdfcomment[author=Charlles]{A unidade de alpha teria que ser (energia/volume)*(volume/mol)^2 = energia*volume/mol^2, não?}
...
This leads us to
\pdfcomment[author=Charlles]{Aqui não deveria haver o subscrito 0 no parâmetro a, certo?}
...
\end{equation}
\pdfcomment[author=Charlles]{Esse phi aparece do nada, sem uma devida descrição. É necessário dizer que tanto phi quanto L são parâmetros ajustáveis.}
and
...
\pdfcomment[author=Charlles]{É necessário deixar mais claro de que artigo (ou artigos) esse equacionamento foi retirado}
...
\pdfcomment[author=Charlles]{Essas modificações são propostas novas ou já foram publicadas por outros autores? Se for uma proposta nova, isso deve ser enfatizado.}
...
\pdfcomment[author=Charlles]{Qual é a razão de os artigos de Kiselev não estarem citados nessa lista?}
...
\pdfcomment[author=Charlles]{Já que existe essa discussão, devemos justificar por que escolhemos considerar CO2 como não-associativo.}
...
Both liquid and vapor densities were included in order to \hl{assure} that the fitted parameters would obey the critical exponent $\beta$ given by \cref{eq:beta_law}.
\pdfcomment[author=Charlles]{Não é possível garantir que o expoente crítico será obedecido.}
...
We observed that the parameters $L$ and $\phi$ present a strong correlation, meaning that several combinations of them give similar results.
A good review on this subject can be found in Bymaster \textit{et al}.
\cite{bymaster2008renormalization}.
The relation is explicit from the derivation of \cref{eq:fns}, following Battle \cite{battlerenorm}, the $\phi$ parameter comes from
\pdfcomment[author=Charlles]{Não entendi o parágrafo. É necessário reescrevê-lo.}
\begin{equation} \label{eq:fns_battle}
\phi = \frac{\psi w^2}{2L^2},
\end{equation}
where $\psi$ is related to the initial shortest wavelength of the density fluctuations and $w$ is \hl{the range of the attractive potential???}.
From~\ref{eq:fns_battle} \hl{it is clear???} that $\phi$ should correspond to some multiple of $L$.
\pdfcomment[author=Charlles]{Não adianta apresentar essa informação sem relacionar com o que foi feito aqui. Se a conclusão é que phi deveria ser proporcional a L, então deveríamos ter estimado 6 parâmetros no total, sendo 5 valores de L e mais a constante de proporcionalidade. Temos, então, que explicar por que isso não foi feito.}
...
From \cref{fig:dfn,fig:PV_evol}, \hl{it is clear that the method converges in four to five iterations}.
\pdfcomment[author=Charlles]{Na verdade, deveríamos ser mais precisos, adotando um critério numérico para convergência. Esse critério "visual" depende da escala adotada para o gráfico.}
...
\caption{Helmholtz-free energy density corrections at each iteration of the renormalization method using the CPA+RG equation of state for methanol at 512.6~K.
	\pdfcomment[author=Charlles]{Vejo vários problemas neste gráfico:
		1) Os "labels" do eixo das densidades se sobrepõem. Recomendo usar notação científica ou alterar a unidade de medida (e.g. 10^3 mol/m^3).
		2) Não há espaço entre "Densidade" e a abertura de parêntese.
		3) O eixo das ordenadas está invertido (isto é, com valores mais negativos acima).
		4) O índice para cada iteração é "n" em df, enquanto o índice que aparece na legenda é "i"}.
	}
\label{fig:dfn}
\end{figure}
...
\caption{Vapor-liquid phase equilibrium envelope for methanol with CPA+RG at consecutive iterations of the renormalization method.
	\pdfcomment[author=Charlles]{Mesmo problemas do gráfico anterior em relação aos "labels" e "titles" e ao índice mostrado na legenda. Também há uma inconsistência, já que tal índice começa em 0, diferentemente do gráfico anterior.}
	}
\label{fig:PV_evol}
\end{figure}
...
\pdfcomment[author=Charlles]{Problemas com esta figura:
	1) Falta espaço antes de abrir parêntese com a unidade de medida.
	2) Onde estão os resultados para etanol?
	3) Por que não apresentar o ponto crítico com símbolo cheio, tal como no gráfico posterior?}
}
\label{fig:pure_LV}
\end{figure}
...
\caption{Critical exponent calculation for methanol, resulting in $\beta = 0.330$ with CPA+RG and $\beta =0.498$ with CPA.
...
\begin{table}[h!]
\centering
\caption{Absolute values of relative deviations (\%) of calculated critical points with respect to experimental data.
\pdfcomment[author=Charlles]{Por que usar valores absolutos? Por que não apresentar o sinal de cada desvio?}
}
\label{table:AAD_crit}
\begin{tabular}{cccccccl} \hline
\multirow{2}{*}{Substance} & \multicolumn{2}{c}{$\displaystyle \bigg|\frac{T_{c}-T_{c}^\text{exp}}{T_{c}^\text{exp}}\bigg|$} & \multicolumn{2}{c}{$\displaystyle \bigg|\frac{P_{c}-P_{c}^\text{exp}}{P_{c}^\text{exp}}\bigg|$} & \multicolumn{2}{c}{$\displaystyle \bigg|\frac{\rho_{c}-\rho_{c}^\text{exp}}{\rho_{c}^\text{exp}}\bigg|$} &  \\ \cline{2-7}
   & CPA& CPA+RG  & CPA   & CPA+RG& CPA & CPA+RG   &  \\ \hline
n-Butane   & 1.49   & 0.85& 13.54 & 6.32  & 8.01& 7.14 &  \\
Methanol   & 4.49   & 0.26& 32.68 & 0.54  & 5.17& 0.43 &  \\
Ethanol& 4.77   & 0.20 & 29.79 & 1.02  & 7.52& 1.39 &  \\
\ce{CO_2}   & 1.91   & 0.53& 11.25 & 3.79  & 9.82& 3.75 &  \\
\ce{H_{2}S}& 3.21   & 0.13& 19.79 & 6.00 & 2.96& 1.86 &  \\ \hline
\end{tabular}
\end{table}

\cref{fig:crit_exp} shows that the CPA+RG presents correct critical exponent $\beta = 0.33$, very close to the universal experimental value, while the CPA equation of state presents a classical mean-field value, $\beta \sim 0.5$.
\pdfcomment[author=Charlles]{As figuras deveriam ser comentadas na ordem em que elas aparecem. Portanto, deve-se postergar este comentário ou adiantar a figura em questão.}
...
\caption{vapor-liquid equilibrium phase envelope for \ce{H_{2}S} + \ce{CO_2} mixture in temperatures 273.15K (below) and 313.15K (above).
Lines are calculated from CPA+RG(\fulline) and CPA(\dashedline).
...
\caption{Speed of sound for methanol at subcritical conditions in the compressed liquid region, symbols are experimental data from NIST database at Tr=0.8 ($\square$) and Tr=0.9 ($\circ$).
Lines are calculated from CPA+RG(\fulline) and CPA(\dashedline)}
...
\caption{Isochoric and isobaric residual heat capacity for methanol at the critical isotherm (512.6K).
...
\caption{Isochoric and isobaric residual heat capacity for methanol at supercritical isotherms, symbols are experimental data from NIST database \cite{nistfluids} at Tr=1.1 ($\circ$) and Tr=1.2 ($\square$).
Lines are calculated from CPA+RG(\fulline) and CPA(\dashedline)}
...
\caption{Speed of sound for methanol at supercritical isotherms, symbols are experimental data from NIST database \cite{nistfluids} at Tr=1.1 ($\circ$) and Tr=1.2 ($\square$).
Lines are calculated from CPA+RG(\fulline) and CPA(\dashedline)}
