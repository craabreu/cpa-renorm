...
Lines were calculated using the CPA+RG method (\fulline) and the CPA equation of state (\dashedline).
\hl{Qual é a causa do comportamento sinuoso da curva verde?}.}
...
Considering the parameter estimation, \hl{only the renormalization parameters do not guarantee} improvements on the representation of isobaric heat capacicies and speed of sound.
In order to model both thermodynamic properties and phase equilibrium in near-critical conditions using equations of state, it might be necessary to do parameter estimation using data from both types, or even re-estimate the equation of state parameters, which are usually obtained from data far away from the critical point only.
As we faced problems \hl{which problems?} with the used parameter estimation method, PSO, a more robust algorithm is indicated \hl{for example?}.

Our results suggest that the used renormalization-group methodology, together with the CPA equation of state, is a better tool than the CPA equation alone to describe phase equilibrium and derivative properties of pure acid gases, as well as binary mixtures containing them, in a large range of conditions, from subcritical up to supercritical ones. \hl{A maior parte dos resultados de puros se refere a metanol.}
