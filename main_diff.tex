...
%DIF DEL main.tex                Thu Apr 19 13:11:34 2018
%DIF ADD main_rev_charlles.tex   Thu Apr  5 20:37:44 2018
...
\linespread{2}
%DIF 8a10-11
\usepackage[author=Charlles]{pdfcomment} %DIF > 
...
\section{Introduction}
	...
	Many unit operations in engineering involve high pressure or high temperature conditions, in which many mixtures approach critical and/or supercritical states. Common examples occur in natural gas processing, in which the natural gas can form mixtures with \ce{CO_2} and \DIFdelbegin \DIFdel{\ce{H_{2}S} }\DIFdelend \DIFaddbegin \DIFadd{\ce{H_2} }\DIFaddend and be found in near- or supercritical states either at reservoir or at transport/operating conditions \DIFdelbegin \DIFdel{~}\DIFdelend \citep{kermani2003carbon}.

    Experimentally, it is observed that fluids exhibit a universal non-classical behavior in the vicinity of critical points, which is evidenced in the so-called critical exponents \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{carles2010brief}}%DIFAUXCMD
. One theoretical explanation is that, at the critical point, the correlation length is in the order of the system size and, because only fluctuations with wavelengths below the correlation length have an expressive impact on the free energy of a fluid, long-wavelength interactions play a major role in the near-critical region. This mechanism gives rise to fluctuations that imply significant changes to the fluids properties}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{carles2010brief}}%DIFAUXCMD
. }\hl{One theoretical explanation is that, at the critical point, the correlation length is in the order of the system size and, because only fluctuations with wavelengths below the correlation length have an expressive impact on the free energy of a fluid, long-wavelength interactions play a major role in the near-critical region, giving rise to fluctuations that imply significant changes to the fluids properties}\pdfcomment{Sentença muito longa}\DIFaddend . Far away from the critical point, instead, the correlation length is in the order of magnitude of the molecule diameters, thus diminishing the influence of long-range interactions and giving the short-range repulsive forces a major role.

    Equations of state are commonly used to model fluids in industrial processes. For instance, the Soave-Redlich-Kwong (SRK) \DIFdelbegin \DIFdel{~}\DIFdelend \cite{soave1972equilibrium} and Peng-Robinson (PR) \DIFdelbegin \DIFdel{~}\DIFdelend \cite{peng1976new} equations of state are used in a wide range of applications. However, for some applications in which molecules are capable of making hydrogen-bonds, both equations present poor performance in representing liquid phases, both for phase equilibrium and derivative properties. In order to overcome this limitation, Kontogeorgis \textit{et al}. \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{kontogeorgis1996equation} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \pdfcomment{tentei usar o comando 'citeauthor' do natbib, mas não funcionou... não descobri o motivo} \DIFadd{\mbox{%DIFAUXCMD
\cite{kontogeorgis1996equation} }%DIFAUXCMD
}\DIFaddend combined the SRK equation of state with Wertheim's first-order thermodynamic perturbation theory (TPT-1), such as in SAFT \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{chapman1990new}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{chapman1990new}}%DIFAUXCMD
}\DIFaddend , and the resulting equation of state (Cubic-Plus-Association, CPA) is capable of representing with better accuracy phase equilibrium and derivative properties of pure substances and mixtures far from critical points.

	\DIFdelbegin \DIFdel{The lack of accuracy of classical equations of state in describing thermodynamic properties in conditions near- and at the critical point }\DIFdelend \DIFaddbegin \hl{Common equations of state, both cubic (e.g. SRK) and non-cubic (e.g. SAFT), are able to describe, respecting their limitations, vapor-liquid equilibrium far from the critical point with acceptable accuracy }\DIFadd{\mbox{%DIFAUXCMD
\cite{orbey1998modeling}}%DIFAUXCMD
. }\hl{However, when pressure and temperature conditions approach the critical state, equations of state fail to describe thermodynamic properties}\DIFadd{. }\pdfcomment{Toda essa parte destacada parece repetir o que já foi dito antes} \DIFadd{This failure }\DIFaddend is evidenced by the fact that \DIFdelbegin \DIFdel{they }\DIFdelend \DIFaddbegin \DIFadd{all classical equations of state }\DIFaddend predict a critical exponent of $1/2$, while the value actually observed experimentally is $1/3$ \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{sengers1974van,wyczalkowska2004critical,stanley1999scaling}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{sengers1974van, wyczalkowska2004critical, stanley1999scaling}}%DIFAUXCMD
}\DIFaddend . Somewhat common in the literature are strategies to improve the description of critical behavior by equations of state based on reestimating parameters, with rather good results being achieved \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{palma2017re}}%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{palma2017re}}%DIFAUXCMD
. }\pdfcomment{Diz-se que é comum obter bons resultandos, mas só se apresenta uma referência} \DIFaddend However, in \DIFdelbegin \DIFdel{some other cases, }\DIFdelend \DIFaddbegin \hl{some other cases}\DIFadd{, }\pdfcomment{quais outros casos?} \DIFaddend the new parameters are able to describe only the vicinity of the critical point, losing accuracy in conditions far from it. In all cases, however, due to the nature of equations of state one cannot correct the scaling of critical phenomena by simply reestimating parameters.

    The equations of state are mean-field theories by nature, and hence do not treat density fluctuations correctly as the fluid approach the critical point. Consequently, the critical exponents which they predict differ from those obtained from experiments. Many theories aim to correct the behavior of classical equations of state. Some methods are based on the hierarchical reference theory \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{parola1984liquid,parola1985hierarchical,meroni1990differential,meroni1993differential}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{parola1984liquid, parola1985hierarchical, meroni1990differential, meroni1993differential}}%DIFAUXCMD
}\DIFaddend . However, these methods have not been frequently developed and applied because of its mathematical complexity. Another category of methods is based on a renormalized Landau expansion of the free energy \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{wyczalkowska2004critical,chen1990crossover,kiselev1991universal,kiselev1998cubic,anisimov1992crossover,kiselev1999cubic,kiselev1999crossover}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{wyczalkowska2004critical, chen1990crossover, kiselev1991universal, kiselev1998cubic, anisimov1992crossover, kiselev1999cubic, kiselev1999crossover}}%DIFAUXCMD
}\DIFaddend , which introduces a \DIFdelbegin \DIFdel{crossover behavior }\DIFdelend \DIFaddbegin \hl{crossover behavior} \pdfcomment{o que isso significa?} \DIFaddend to the equations of state, turning them into non-analytical models with many adjustable parameters. The last category of methods, developed by \DIFdelbegin \DIFdel{White and Zhang~\mbox{%DIFAUXCMD
\cite{white1993renormalization,white1995renormalization,white1998renormalization}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \hl{White and Zhang} \pdfcomment{Só se usa "et al." quando há três ou mais autores} \DIFadd{\mbox{%DIFAUXCMD
\cite{white1993renormalization, white1995renormalization, white1998renormalization}}%DIFAUXCMD
}\DIFaddend , relies on the renormalization group (RG) theory via Phase Space Cell Approximation \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{wilson1971renormalization, wilson1971brenormalization}}%DIFAUXCMD
}\DIFaddend . This method introduces a recursive procedure intended to renormalize the free energy obtained from equations of state. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < 	%%%
\DIFdelend With this approach, it is possible to treat the problem of fluctuations in different scales, evolving from fluctuations in the microscopic scale up to the macroscopic measured scale in a recursive manner. Thus, it is possible, using common equations of state, to better represent phase equilibrium and thermodynamic properties of pure substances and mixtures in conditions that comprehend near-critical and supercritical states.

	Since its first application  \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{white1993renormalization}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{white1993renormalization}}%DIFAUXCMD
}\DIFaddend , White and Zhang\DIFdelbegin \DIFdel{’}\DIFdelend \DIFaddbegin \DIFadd{'}\DIFaddend s recursive method has been improved by a number of authors. The same authors extended the range in which the method could be applied  \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{white1998renormalization}}%DIFAUXCMD
, }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{white1998renormalization}}%DIFAUXCMD
. }\DIFaddend Lue and Prausnitz \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{lue1998renormalization,lue1998brenormalization} }%DIFAUXCMD
and Mi et al. ~}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{lue1998renormalization, lue1998brenormalization} }%DIFAUXCMD
and Mi }\textit{\DIFadd{et al}}\DIFadd{. }\DIFaddend \cite{mi2004improved} improved the method so that is can be applied to the whole range of temperature, even far away from the critical point. \DIFdelbegin \DIFdel{Up to this publication}\DIFdelend \DIFaddbegin \hl{Up to this publication} \pdfcomment{Qual publicação? A nossa? Se for, não precisa dizer.}\DIFaddend , many authors have applied this method using different mean-field theories (e.g. MSA \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{lue1998renormalization}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{lue1998renormalization}}%DIFAUXCMD
}\DIFaddend , equation of state for chain fluids \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{jiang1999equation,jiang2000phase}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{jiang1999equation, jiang2000phase}}%DIFAUXCMD
}\DIFaddend , SAFT equation of state and its variations \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{llovell2004thermodynamic,forte2011application,bymaster2008renormalization,tang2010renormalization,llovell2006global,forte2013application,dias2009thermodynamic,llovell2006second,llovell2006prediction,llovell2007phase}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{llovell2004thermodynamic, forte2011application, bymaster2008renormalization, tang2010renormalization, llovell2006global, forte2013application, dias2009thermodynamic, llovell2006second, llovell2006prediction, llovell2007phase}}%DIFAUXCMD
}\DIFaddend , cubic equations of state  \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{xu2011prediction,xu2010crossover,pcm2017application,llovell2008accurate,qiu2006vapor,cai2006vapor,cai2004thermodynamics}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{xu2011prediction, xu2010crossover, pcm2017application, llovell2008accurate, qiu2006vapor, cai2006vapor, cai2004thermodynamics}}%DIFAUXCMD
}\DIFaddend , and others \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{ghobadi2013renormalization,choi2016renormalization}}%DIFAUXCMD
)}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{ghobadi2013renormalization, choi2016renormalization}}%DIFAUXCMD
), }\DIFaddend obtaining similar results. However, some aspect in the method are still unclear, e.g. parameter estimation procedures and approximations in the renormalization equations. There is still need for further assessment of the method's capabilities and limitations.

	This work aims at evaluating the impacts on modeling phase equilibrium and derivative properties when applying the renormalization group theory with the CPA equation of state, referred to as the CPA+RG approach for short. We studied pure substances and different mixtures including \ce{CO_2} and \DIFdelbegin \DIFdel{\ce{H_{2}S}}\DIFdelend \DIFaddbegin \DIFadd{\ce{H_2S}}\DIFaddend , in conditions near-to and far from the critical point. We provide a parameter estimation procedure that improves the methodology.
...
\DIFaddbegin 
    ...
    \DIFaddend Pure substances and binary mixtures in vapor-liquid equilibrium and at supercritical conditions were evaluated using the Cubic-Plus-Association equation of state \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{kontogeorgis1996equation}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{kontogeorgis1996equation}}%DIFAUXCMD
}\DIFaddend . In the pressure-explicit form, such equation reads
\begin{equation} \label{eq:pressure_cpa}
P = \frac{RT}{v-b}-\frac{a}{v(v+b)}-\frac{1}{2}\frac{RT}{v} \DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Big(\DIFaddend 1+\rho\frac{\partial \ln g}{\partial \rho}\DIFdelbegin %DIFDELCMD < \right)%%%
\DIFdelend \DIFaddbegin \Big)\DIFaddend \sum_{i} x_{i} \sum_{A_{i}}(1-X_{A_{i}})\DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation}
    where $P$ is the pressure, $R$ is the ideal gas constant, $v$ is the molar volume, $b$ is the mixture co-volume parameter, $a$ is the mixture energy parameter, $T$ is the temperature, $x_{i}$ is the mole fraction of the $i$-th component, $A_{i}$ is the association site $A$ of the $i$-th component\DIFaddbegin \DIFadd{, }\DIFaddend and $X_{A_{i}}$ is the fraction of non-associated $A$ sites of the \DIFdelbegin \textit{\DIFdel{i}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{$i$}\DIFaddend -th component. As the renormalization procedure calculates corrections explicitly to the Helmholtz free energy density, it is useful to represent the CPA equation of state in its Helmholtz-explicit form, given by
\begin{subequations}
...
\bar{a}_{assoc}^{res} = RT\sum_{i=1} x_{i} \sum_{A_{i}}\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Big(\DIFaddend \ln X_{A_{i}} - \frac{X_{A_{i}}}{2} + \frac{1}{2}\DIFdelbegin %DIFDELCMD < \right)%%%
...
\begin{equation}
	...
	where $k_{ij}$ is a binary interaction parameter (here considered as zero for all examples) and $nc$ is the number of components in the mixture. The energy parameter of each component $a_{i}$ is calculated using a Soave-type temperature dependency, which is \DIFaddbegin \pdfcomment{Em vez de usar 'big' para aumentar parênteses e colchetes, é melhor usar 'left' e 'right' para que o latex determine automaticamente o tamanho}
\DIFaddend \begin{equation}
a_{i}(T) = a_{0_{i}}\left[1+c_{1_{i}}\left(1-\sqrt{\frac{T}{T_{c_{i}}}}\right)\right]^2\DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation}
	where $a_{0_{i}}$, $b_{i}$, $c_{1_{i}}$ are parameters of the CPA equation. The last term of Eq.~\ref{eq:pressure_cpa} \DIFaddbegin \pdfcomment{No latex, o til (~) representa um 'espaço não separável'... ele perde sua função se for precedido ou sucedido por um espaço} \DIFaddend is the association term, in which $X_{A_{i}}$ represents the fractions of non-associated $A$ sites of the \DIFdelbegin \textit{\DIFdel{i}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{$i$}\DIFaddend -th component, and can be calculated by
\begin{equation} \label{eq:frac_nb}
    X_{A_{i}} = \DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Big(\DIFaddend 1+\rho \sum_{j}x_{j} \sum_{B_{j}} X_{B_{j}} \Delta^{A_{i}B_{j}}\DIFdelbegin %DIFDELCMD < \right)%%%
\DIFdelend \DIFaddbegin \Big)\DIFaddend ^{-1}\DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation}
    in which $\Delta^{A_{i}B_{j}}$ is the association strength, given by \DIFaddbegin \pdfcomment{existem comandos no latex para funções conhecidas, tais como exp, ln, sin, cos etc.}
\DIFaddend \begin{equation} \label{eq:delta_cpa}
    \Delta^{A_{i}B_{j}} = g(\rho)\DIFdelbegin %DIFDELCMD < \left[%%%
\DIFdelend \DIFaddbegin \Big[\DIFaddend \exp\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \bigg(\DIFaddend \frac{\epsilon^{A_{i}B_{j}}}{RT}\DIFdelbegin %DIFDELCMD < \right)%%%
\DIFdelend \DIFaddbegin \bigg)\DIFaddend -1\DIFdelbegin %DIFDELCMD < \right]%%%
\DIFdelend \DIFaddbegin \Big]\DIFaddend b_{ij}\beta^{A_{i}B_{j}},
\end{equation}
with
\begin{equation} \label{eq:bij}
    b_{ij} = \DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Big(\DIFaddend \frac{b_{i}+b{j}}{2}\DIFdelbegin %DIFDELCMD < \right)
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \Big)\DIFadd{,
}\DIFaddend \end{equation}
\begin{equation} \label{eq:g_cpa}
    g(\rho) = \frac{1}{1-1.9\eta}, \quad \text{and}
\end{equation}
\begin{equation} \label{eq:eta_cpa}
    \eta = \frac{b\rho}{4}\DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation}
	where $\beta_{i}^{A_{i}B_{j}}$ and $\epsilon_{i}^{A_{i}B_{j}}$ are the association parameters of the CPA equation, $g(\rho)$ is the contact value of the radial distribution function\DIFaddbegin \DIFadd{, }\DIFaddend and $\eta$ is the packing fraction. The version of the CPA equation of state presented here is commonly referred to in the literature as the simplified-CPA (s-CPA) equation \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{kontogeorgis1999multicomponent}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{kontogeorgis1999multicomponent}}%DIFAUXCMD
}\DIFaddend , but in this work we will simply use the \DIFdelbegin \DIFdel{term }\DIFdelend \DIFaddbegin \DIFadd{therm }\DIFaddend CPA for simplicity. In this work, $X_{A_{i}}$ is calculated following the numerical procedure proposed by Michelsen \textit{et al}. \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{michelsen2006robust}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{michelsen2006robust}}%DIFAUXCMD
}\DIFaddend .

	The recursive method developed by White and Zhang \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{white1993renormalization}}%DIFAUXCMD
,~\mbox{%DIFAUXCMD
\citep{white1995renormalization}}%DIFAUXCMD
,~\mbox{%DIFAUXCMD
\citep{white1998renormalization}}%DIFAUXCMD
, }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{white1993renormalization, white1995renormalization, white1998renormalization} }%DIFAUXCMD
}\DIFaddend and by Salvino and White \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{salvino1992calculation} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{salvino1992calculation} }%DIFAUXCMD
}\DIFaddend introduces density fluctuations to \DIFdelbegin \DIFdel{a given framework }\DIFdelend \DIFaddbegin \hl{a given framework} \pdfcomment{Esse termo é meio vago.} \DIFaddend of Helmholtz energy density. The method is based on the Phase Space Cell Approximation method developed by Wilson \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{wilson1971renormalization}}%DIFAUXCMD
,~\mbox{%DIFAUXCMD
\citep{wilson1971brenormalization}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{wilson1971renormalization,  wilson1971brenormalization}}%DIFAUXCMD
}\DIFaddend , where each iteration accounts for fluctuations of longer and longer wavelengths, thus \DIFdelbegin \DIFdel{, }\DIFdelend introducing a non-analytic correction to the analytical equation of state. An illustration of the method is represented in Fig.~\ref{fig:schematics}.
...
f_{0}(\rho) = \rho RTln(\frac{\rho}{1-\rho b})-\frac{a\rho}{b}ln(1+\rho b) + \rho RT\sum_{i=1} x_{i} \sum_{A_{i}}\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Big(\DIFaddend lnX_{A_{i}} - \frac{X_{A_{i}}}{2} + \frac{1}{2}\DIFdelbegin %DIFDELCMD < \right) %%%
...
\begin{equation} \label{eq:dfn}
    df_{n}(\rho) = -K_{n} ln\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Bigg(\DIFaddend \frac{\Omega_{n}^{s}(\rho)}{\Omega_{n}^{l}(\rho)}\DIFdelbegin %DIFDELCMD < \right)
%DIFDELCMD < %%%
...
\DIFaddend \begin{equation} \label{eq:Omega}
    \Omega_{n}^{\gamma} = \int_{0}^{min(\rho,\rho_{max}-\rho)}exp\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Bigg(\DIFaddend -\frac{G_{n}^{\gamma}(\rho,x)}{K_{n}}\DIFdelbegin %DIFDELCMD < \right)%%%
...
\DIFdelbegin \DIFdel{where }\DIFdelend \DIFaddbegin 
    ...
    \DIFadd{Where }\DIFaddend $f_{n}^{s}$ and $f_{n}^{l}$ are modified Helmholtz free energy terms, related to \DIFdelbegin \DIFdel{short-wavelength fluctuations }\DIFdelend \DIFaddbegin \DIFadd{short- }\DIFaddend and long-wavelength fluctuations\DIFaddbegin \DIFadd{, }\DIFaddend respectively.
...
\end{equation}
    ...
    Here we considered $f_{5}$ to be the final Helmholtz free energy density. To apply the renormalization procedure to mixtures, two major approximations were used. First, the isomorphic approximation \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{fisher1968renormalization} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{fisher1968renormalization} }%DIFAUXCMD
}\DIFaddend was used instead of the Phase Space Cell Approximation. The main difference between the two approximations is that the isomorphic approximation considers that the total density of the fluid is used to consider fluctuations, whilst the Phase Space Cell Approximation implies that each individual component in the mixture fluctuates independently. Second, as the isomorphism assumption requires that the chemical potential be the isomorphic variable in the present case, which is not a common independent variable for mixture phase equilibria modelling, following Kiselev and Friend \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{kiselev1999cubic}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{kiselev1999cubic}}%DIFAUXCMD
}\DIFaddend , we used mole fractions as isomorphic variables instead of the individual chemical potentials. These two approximations make possible to extend equations ~\cref{eq:f00,eq:fn,eq:dfn,eq:Kn,eq:Omega,eq:Gn,eq:fns,eq:fnl} with the need of only mixing rules (~\cref{eq:mix_L,eq:mix_phi}) for the renormalization parameters
    \DIFaddbegin 

\DIFaddend \begin{equation} \label{eq:mix_L}
...
\end{equation}
    ...
    These approximations were used by other authors with good results \DIFdelbegin \DIFdel{~}\DIFdelend \cite{cai2004thermodynamics,llovell2006global,sun2005application,pcm2017application,xu2011prediction}. Tang and Gross  \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{tang2010renormalization} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{tang2010renormalization} }%DIFAUXCMD
}\DIFaddend compared the Phase Space Cell Approximation and Isomorphic Approximation using the PC-SAFT equation of state, and concluded that the phase-space cell approximation present similar performance to the isomorphic approximation. 

    The substances analyzed here are: n-butane, n-octane, methanol, ethanol, \ce{CO_2}, and \DIFdelbegin \DIFdel{\ce{H_{2}S}}\DIFdelend \DIFaddbegin \DIFadd{\ce{H_2S}}\DIFaddend . These substances were chosen because of the different associative combinations that could be evaluated. n-Butane and \ce{CO_2} were considered non-associative substances, while methanol, ethanol, and \DIFdelbegin \DIFdel{\ce{H_{2}S} }\DIFdelend \DIFaddbegin \DIFadd{\ce{H_2S} }\DIFaddend are considered associative substances. There is a large discussion on the literature on the most adequate associating models \textbf{and considerations} to represent these substances, mainly \ce{CO_2}  \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{bjorner2016modeling} }%DIFAUXCMD
and \ce{H_{2}S} ~\mbox{%DIFAUXCMD
\citep{ruffine2006represent}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{bjorner2016modeling} }%DIFAUXCMD
and \ce{H_2S}  \mbox{%DIFAUXCMD
\cite{ruffine2006represent}}%DIFAUXCMD
}\DIFaddend . A common example is whether \ce{CO_2} should or not be considered an associating molecule and further, whether the extent of quadrupole forces inclusion is important \DIFdelbegin \DIFdel{~}\DIFdelend \cite{tsivintzelis2011modeling,bjorner2016modeling}.
...
\makebox[\textwidth]{\begin{tabular}{lllllll}
                                                                                                              \hline
Substance   & Association Model & $a_{0_{i}}$ ($Pa.m^{6}.mol$) & $b_{i}$ ($10^{5}\times m^{3}.mol^{-1}$) & $c_{1_{i}}$      & $\epsilon_{i}^{A_{i}B_{j}}$ ($MPa.m^{3}.mol^{-1}$) & $\beta_{i}^{A_{i}B_{j}}$  \\ \hline
...
\DIFdelbeginFL \DIFdelFL{\ce{H_{2}S}         }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\ce{H_2S}         }\DIFaddendFL & 4C                & 0.32            & 2.93         & 0.82805 & 0.00146              & 0.486  \\ \hline     
\end{tabular}}
...
\begin{table}[h!]
\centering
\caption{Experimental critical data obtained from NIST database \DIFdelbeginFL \DIFdelFL{~}\DIFdelendFL \cite{nistfluids} and used as reference}
...
\DIFdelbeginFL \DIFdelFL{\ce{H_{2}S}         }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\ce{H_2S}         }\DIFaddendFL & 373.4  & 8.960    & 10200         \\ \hline     
\end{tabular}}
\end{table}
...
O.F.=\sum_{i=1}^{n_{exp}} \DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Bigg(\DIFaddend \frac{\rho_{i}^{liq,exp}-\rho_{i}^{liq,calc}}{\rho_{i}^{liq,exp}}\DIFdelbegin %DIFDELCMD < \right)%%%
\DIFdelend \DIFaddbegin \Bigg)\DIFaddend ^2 + \sum_{i=1}^{n_{exp}} \DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Bigg(\DIFaddend \frac{\rho_{i}^{vap,exp}-\rho_{i}^{vap,calc}}{\rho_{i}^{vap,exp}}\DIFdelbegin %DIFDELCMD < \right)%%%
\DIFdelend \DIFaddbegin \Bigg)\DIFaddend ^2 + \sum_{i=1}^{n_{exp}} \DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Bigg(\DIFaddend \frac{P_{i}^{sat,exp}-P_{i}^{sat,calc}}{P_{i}^{sat,exp}}\DIFdelbegin %DIFDELCMD < \right)%%%
\DIFdelend \DIFaddbegin \Bigg)\DIFaddend ^2
\end{equation}
...
Ethanol     & 5.48  & 0.75          \\
\ce{CO_2}         & 4.44  & 2.62         \\
\DIFdelbeginFL \DIFdelFL{\ce{H_{2}S}         }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\ce{H_2S}         }\DIFaddendFL & 4.30  & 2.40         \\ \hline     
\end{tabular}}
\end{table}

	We observed that the parameters $L$ and $\phi$ present a strong correlation, that mean several combinations of them give similar results. A good review on this subject can be found in Bymaster et al. \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{bymaster2008renormalization}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{bymaster2008renormalization}}%DIFAUXCMD
}\DIFaddend . The relation is explicit from the derivation of equation~\ref{eq:fns}, following Battle \DIFdelbegin \DIFdel{~}\DIFdelend \cite{battlerenorm}, the $\phi$ parameter comes from
...
\end{figure}
    ...
    Results concerning phase equilibria calculations are summarized in Figures ~\ref{fig:pure_LV}, ~\ref{fig:sat_p}, ~\ref{fig:crit_exp}, ~\ref{fig:pure_isotherm} and Table ~\ref{table:AAD_crit}.

\begin{figure}[h!]
\centering
\captionsetup{justification=centering}
\makebox[\textwidth][c]{\includegraphics[width=1.0\textwidth]{PV_pure_4.png}}
\caption{vapor-liquid phase envelope. Lines are calculated from CPA+RG(\fulline) and CPA(\dashedline). Experimental Data extracted from NIST database \DIFdelbeginFL \DIFdelFL{~}\DIFdelendFL \cite{nistfluids} ($\square$)}
\label{fig:pure_LV}
\end{figure}
...
\caption{Saturation pressure curves. Lines are calculated from CPA+RG(\fulline) and CPA(\dashedline). Experimental Data extracted from NIST database \DIFdelbeginFL \DIFdelFL{~}\DIFdelendFL \cite{nistfluids} ($\square$)}, critical points are represented by filled symbols.
\label{fig:sat_p}
\end{figure}
...
\section{Binary Mixture Phase Equilibria Results}
    ...
    In all binary mixtures, we evaluated both subcritical and critical conditions for three kinds of mixture: 2 non-associating components (\ce{CO_2} + n-butane); 1 associating and 1 non-associating component (\DIFdelbegin \DIFdel{\ce{H_{2}S} }\DIFdelend \DIFaddbegin \DIFadd{\ce{H_2S} }\DIFaddend + \ce{CO_2}); 2 associating components (\DIFdelbegin \DIFdel{\ce{H_{2}S} }\DIFdelend \DIFaddbegin \DIFadd{\ce{H_2S} }\DIFaddend + Methanol). In all cases the CR-1 combining rule is used. Solvating phenomena is not considered for the case of \DIFdelbegin \DIFdel{\ce{H_{2}S} }\DIFdelend \DIFaddbegin \DIFadd{\ce{H_2S} }\DIFaddend + \ce{CO_2}.

\begin{figure}[h!]
\centering
\captionsetup{justification=centering}
\makebox[\textwidth][c]{\includegraphics[width=0.75\textwidth]{bin_co2_c4.png}}
\caption{vapor-liquid Phase Equilibria Envelope to the mixture \ce{CO_2} + n-butane in the temperatures of 260K (below) and 344.3K (above). Lines are calculated from CPA+RG(\fulline) and CPA(\dashedline). Experimental data extracted from Clark et al. \DIFdelbeginFL \DIFdelFL{~}\DIFdelendFL \cite{clark1988vapour+} ($\circ$) and Hsu et al. \DIFdelbeginFL \DIFdelFL{~}\DIFdelendFL \cite{hsu1985equilibrium} ($\square$)}
...
\caption{vapor-liquid equilibria phase envelope for \DIFdelbeginFL \DIFdelFL{\ce{H_{2}S} }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\ce{H_2S} }\DIFaddendFL + Methanol mixture in temperatures 298.15K (below) and 448.15K (above). Lines are calculated from CPA+RG(\fulline) and CPA(\dashedline). Squares Experimental data extracted from Leu et al.  \DIFdelbeginFL \DIFdelFL{~}\DIFdelendFL \cite{leu1992equilibrium} ($\square$,$\circ$)}
...
\caption{vapor-liquid equilibria phase envelope for \DIFdelbeginFL \DIFdelFL{\ce{H_{2}S} }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\ce{H_2S} }\DIFaddendFL + \ce{CO_2} mixture in temperatures 273.15K (below) and 313.15K (above). Lines are calculated from CPA+RG(\fulline) and CPA(\dashedline). Experimental data extracted from Bierlein et al.  \DIFdelbeginFL \DIFdelFL{~}\DIFdelendFL \cite{bierlein1953phase} ($\square$,$\circ$)}
...
\caption{Isochoric and isobaric residual heat capacity for methanol at subcritical conditions in the compressed liquid region, symbols are experimental data from NIST database \DIFdelbeginFL \DIFdelFL{~}\DIFdelendFL \cite{nistfluids} at Tr=0.8 ($\square$) and Tr=0.9 ($\circ$). Lines are calculated from CPA+RG(\fulline) and CPA(\dashedline)}
...
\caption{Isochoric and isobaric residual heat capacity for methanol at the critical isotherm (512.6K). Experimental data from NIST database \DIFdelbeginFL \DIFdelFL{~}\DIFdelendFL \cite{nistfluids} ($\square$). Lines are calculated from CPA+RG(\fulline) and CPA(\dashedline)}
...
\caption{Speed of sound for methanol at the critical isotherm (512.6K). Experimental data from NIST database \DIFdelbeginFL \DIFdelFL{~}\DIFdelendFL \cite{nistfluids} ($\square$). Lines are calculated from CPA+RG(\fulline) and CPA(\dashedline)}
...
\begin{equation} \label{eq:chem_pot}
    u = \DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Big(\DIFaddend \frac{\partial f}{\partial \rho}\DIFdelbegin %DIFDELCMD < \right)%%%
...
\begin{equation} \label{eq:press_helm_deriv}
	P = -\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Bigg(\DIFaddend \frac{\partial A}{\partial V}\DIFdelbegin %DIFDELCMD < \right)%%%
...
\begin{equation} \label{eq:press_helm_deriv2}
	P = -f+\rho\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Bigg(\DIFaddend \frac{\partial f}{\partial \rho}\DIFdelbegin %DIFDELCMD < \right)%%%
\DIFdelend \DIFaddbegin \Bigg)\DIFaddend _{T,\textbf{n}}
\end{equation}
...
\begin{equation} \label{eq:chem_pot_i}
	\mu_{i}^{res} = \DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Bigg(\DIFaddend \frac{\partial f^{res}}{\partial \rho{*}}\DIFdelbegin %DIFDELCMD < \right)%%%
\DIFdelend \DIFaddbegin \Bigg)\DIFaddend _{T,x_{k\neq j,nc}}b_{i}+\sum_{j=1}^{nc} \DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Bigg(\DIFaddend \frac{\partial f^{res}}{\partial x_{j}}\DIFdelbegin %DIFDELCMD < \right)%%%
\DIFdelend \DIFaddbegin \Bigg)\DIFaddend _{T,\rho b,x_{k\neq j,nc}} \DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Bigg(\DIFaddend \frac{\delta_{ij} \rho_{i}-\rho_{j}}{\rho^{2}}\DIFdelbegin %DIFDELCMD < \right)
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \Bigg)
\DIFaddend \end{equation} 
...
\begin{equation} \label{eq:fugacity}
	ln\phi_{i} = \frac{\mu_{i}^{res}}{RT} - ln\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Bigg(\DIFaddend \frac{P}{RT\rho}\DIFdelbegin %DIFDELCMD < \right)
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \Bigg)
\DIFaddend \end{equation}
...
\begin{equation} \label{eq:Cvres_deriv}
	C_{v}^{res} = -T\DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Bigg(\DIFaddend \frac{\partial^{2}\bar{a}}{\partial T^{2}}\DIFdelbegin %DIFDELCMD < \right)%%%
...
\begin{equation} \label{eq:Cpres_deriv}
	C_{p}^{res} = C_{v}^{res} + T \DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Bigg(\DIFaddend \frac{\partial P}{\partial T}\DIFdelbegin %DIFDELCMD < \right)%%%
\DIFdelend \DIFaddbegin \Bigg)\DIFaddend _{V,\textbf{n}} \DIFdelbegin %DIFDELCMD < \left(%%%
\DIFdelend \DIFaddbegin \Bigg(\DIFaddend \frac{\partial v}{\partial T}\DIFdelbegin %DIFDELCMD < \right)%%%
